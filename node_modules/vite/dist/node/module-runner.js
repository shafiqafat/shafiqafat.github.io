...
function decode(mappings) {
	...
	do {
		...
		for (genColumn = 0; reader.pos < semi;) {
			let seg;
			genColumn += decodeSign(decodeInteger(reader)), genColumn < lastCol && (sorted = !1), lastCol = genColumn, hasMoreVlq(reader, semi) ? (sourcesIndex += decodeSign(decodeInteger(reader)), sourceLine += decodeSign(decodeInteger(reader)), sourceColumn += decodeSign(decodeInteger(reader)), hasMoreVlq(reader, semi) ? (namesIndex += decodeSign(decodeInteger(reader)), seg = [
				genColumn,
				sourcesIndex,
				sourceLine,
				sourceColumn,
