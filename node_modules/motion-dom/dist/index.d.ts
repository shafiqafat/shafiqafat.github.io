 ...
 * EffectSubject.
 */
interface AnimationElement extends Omit<WithRender, "current">, Owner {
    KeyframeResolver?: typeof KeyframeResolver;
    shouldSkipAnimations?: boolean;
    shouldReduceMotion?: boolean | null;
}
 ...
 */
type MotionValueAnimation = Pick<AnimationPlaybackControlsWithThen, "state" | "stop" | "finished" | "then">;
...
type ResolvedOptions<T extends AnyResolvedKeyframe> = ValueAnimationOptions<T> & {
    startTime?: number;
    finalKeyframe?: T;
};
declare class AsyncMotionValueAnimation<T extends AnyResolvedKeyframe> extends WithPromise implements AnimationPlaybackControls {
    ...
    onKeyframesResolved(keyframes: ResolvedKeyframes<T>, finalKeyframe: T, options: ResolvedOptions<T>, sync: boolean): void;
...
declare const animateMotionValue: <V extends AnyResolvedKeyframe>(name: string, value: MotionValue<V>, target: V | UnresolvedKeyframes<V>, transition?: ValueTransition & {
    elapsed?: number;
}, element?: AnimationElement, isHandoff?: boolean) => StartAnimation;
...
declare class MotionValueState {
     ...
     */
    set(name: string, value: MotionValue, render?: VoidFunction, computed?: MotionValue): VoidFunction;
     ...
     * values bound later.
     */
    release(): Map<string, MotionValue<any>>;
    private schedule;
    private cancel;
    /**
...
interface EffectKeyframes {
    [key: string]: UnresolvedValueKeyframe | UnresolvedValueKeyframe[] | Record<string, unknown> | undefined;
 ...
 */
type EffectTransition = ValueTransition & {
    reduceMotion?: boolean;
    skipAnimations?: boolean;
 ...
 */
declare function animateValues(getValue: (key: string) => MotionValue, keyframes: EffectKeyframes, transition?: EffectTransition, element?: AnimationElement): AnimationPlaybackControlsWithThen[];
/**
 ...
 */
declare function animateEffectSubject<Subject extends object>(effect: AnimateEffect<Subject>, subject: Subject, keyframes: EffectKeyframes, transition?: EffectTransition, element?: AnimationElement): AnimationPlaybackControlsWithThen[];
 ...
 */
interface SelectorEffect<T extends object> extends EffectOptions<T> {
    (subject: ElementOrSelector, values: Record<string, MotionValue>): VoidFunction;
...
interface SelectorAnimateEffect<T extends object> extends SelectorEffect<T> {
    test: EffectTest<T>;
    read: EffectRead<T>;
}
 ...
 */
declare const readStyleValue: (element: StyleSubject, key: string) => string | number;
/**
 ...
 */
declare const styleSubjectEffect: AnimateEffect<StyleSubject>;
declare const styleEffect: SelectorAnimateEffect<StyleSubject>;
 ...
 */
declare function animateElement(element: StyleSubject, keyframes: EffectKeyframes, transition?: EffectTransition, visualElement?: VisualElement): AnimationPlaybackControlsWithThen[];
/**
 ...
 */
declare function handOffElementState(element: Element, visualElement: VisualElement): void;
 ...
 */
declare const addAttrValue: (element: HTMLElement | SVGElement, state: MotionValueState, key: string, value: MotionValue, name?: string) => VoidFunction;
declare const attrEffect: SelectorEffect<HTMLElement | SVGElement>;
 ...
 */
declare const propEffect: AnimateEffect<PropSubject>;

declare const addSVGValue: (element: SVGElement, state: MotionValueState, key: string, value: MotionValue) => VoidFunction;
/**
 ...
 * (`baseFrequency`).
 */
declare const readSVGValue: (element: SVGElement, key: string) => any;
/**
 * The per-element effect `animate()` binds through, so SVG values share
 * state with a direct `svgEffect()` call.
 */
declare const svgSubjectEffect: AnimateEffect<SVGElement>;
declare const svgEffect: SelectorAnimateEffect<SVGElement>;
 ...
 */
type FollowValueOptions = Omit<ValueAnimationTransition, "onUpdate" | "onComplete" | "onPlay" | "onRepeat" | "onStop" | "repeat" | "repeatType" | "repeatDelay"> & {
    /**
 ...
 */
declare function hasComplexValues(value: AnyResolvedKeyframe): boolean;
